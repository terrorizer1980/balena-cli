...
function originalTarDirectory(dir, param) {
	...
	const getFiles = () => {
		// @ts-expect-error The stream-to-promise types do not handle object-mode streams like klaw properly, hence the cast
		// prettier-ignore
		const p = /** @type {Promise<klaw.Item[]>} */ (streamToPromise(klaw(dir)))
